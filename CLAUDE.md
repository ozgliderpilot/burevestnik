...
```
main.py  →  scrape.fetch(url)         →  parse.extract(html, day=…)  →  caption.render(forecast, now, url)  →  telegram.send_photo()
            (Playwright I/O,             (pure, HTML→Forecast)           (pure, Forecast→HTML-formatted str)      (httpx POST,
             returns html + jpeg)                                                                                  HTML parse_mode)
```

`forecast_day(now)` (in `models.py`) is the only path through the 16:00 `FORECAST_TZ` cutoff; it returns a **Forecast day** (Today or Tomorrow, plus that day's date, plus a derived meteoblue page index). `main.py` computes that value once, shapes the fetch URL via `forecast_page_url` (Tomorrow appends `?day=2`; the unmodified URL is what the caption links to), and passes the same value into `parse.extract`. `Forecast` carries it as `day`, so `caption.render` has no parallel flag. `scrape.py` itself is URL-agnostic — it opens whatever it's given.

Two key invariants make the parser tractable:

1. **`scrape.py` is the only module with browser side effects.** It pre-seeds the `temp` / `speed` / `precip` cookies on `www.meteoblue.com` so the page renders in `CELSIUS / KNOT / MILLIMETER` regardless of the runner's geo-IP — this is what makes `parse.py` regex-based and geo-IP-independent. (Cookie-based unit setting also avoids losing query strings like `?day=2`: meteoblue's settings-menu unit anchors point at the bare base URL, so clicking them after a `?day=2` fetch would silently revert the hourly table to today.) It then dismisses the GDPR overlay, flips the table to the 1-hour view via `label.switch-with-label`, and screenshots `table.hourlywind`.
...
`Forecast` carries a Forecast day plus that day's summary and hourly-table-derived metrics: peak rain (mm + `HH:00`), felt temp range, wind range, peak gust, UV index, and sunrise/sunset. Each metric is extracted by a dedicated `parse_*` helper that reads one row of `table.hourlywind` (or a sibling page-level block for UV / sun times). They reflect whichever day the page was fetched for — `?day=2` swaps the table to tomorrow at the URL level — so they need no extra flag. When Forecast day is Tomorrow the `next_day_preview` field is `None` (no day-after teaser at end-of-day).
...
### Monday/Thursday 5-day outlook

On Monday and Thursday mornings (gated by the pure `main.should_post_outlook(now)` — weekday is Mon/Thu and Forecast day is Today), `main()` posts one **extra** photo *before* the normal daily forecast. `main._post_outlook` orchestrates it: `scrape.fetch_meteogram(url)` screenshots the "Hourly weather forecast" meteogram (`#blooimage`, a lazy-loaded Highcharts SVG — it must be scrolled into view to hydrate) cropped vertically to just its **temperature panel** via `_TEMP_PANEL_TOP_FRACTION`/`_TEMP_PANEL_BOTTOM_FRACTION` of the element height; `parse.parse_days(html, 5)` reads `#day1`…`#day5` into a `list[DaySummary]` (the page carries 14 day tabs; `#day1` is always today); and `caption.render_outlook(days, now, url)` renders a compact one-line-per-day caption (condition emoji · weekday · hi/lo · rain · sun hours; wind omitted; no header line; same `Updated …/meteoblue` footer). The branch is **best-effort** (wrapped in `try/except` in `main()`) so a meteogram failure logs a warning and never blocks the daily forecast that follows. `scrape.fetch` and `fetch_meteogram` share browser/cookie/consent setup via the `_browser_page` context manager.
